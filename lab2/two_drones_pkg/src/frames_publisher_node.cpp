 ...
 public:
  ...
  void onPublish() {
    ...
    //   - convert the resulting Duration to seconds, store result into a double
      ...
      double time_diff = (curr_time - startup_time).seconds();
      RCLCPP_DEBUG_STREAM(this->get_logger(), "TIme diff " << time_diff);
    ...
    // ...
      tf2::Quaternion q;
      q.setRPY(0, 0, time_diff);
      RCLCPP_DEBUG_STREAM(this->get_logger(), "Quaternion" << q.w());
      world_T_av1.header.stamp = this->get_clock()->now();
      world_T_av1.header.frame_id = "world";
      world_T_av1.child_frame_id ="av1";
      world_T_av1.transform.translation.x = cos(time_diff);
      world_T_av1.transform.translation.y = sin(time_diff);
      ...
      RCLCPP_DEBUG_STREAM(this->get_logger(), "World - av1" << world_T_av1.transform.translation.x);


      world_T_av2.header.stamp = this->get_clock()->now();
      world_T_av2.header.frame_id = "world";
      world_T_av2.child_frame_id ="av2";
      world_T_av2.transform.translation.x = sin(time_diff);
      world_T_av2.transform.translation.y = 0.0;
      world_T_av2.transform.translation.z = cos(2*time_diff);
